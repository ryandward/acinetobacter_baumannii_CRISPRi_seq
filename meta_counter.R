...
aba <- fread("all_counts_seal.tsv.gz",
						 header = FALSE,
...
aba_genome <- aba_bed[aba_key[, .(spacer, type, locus_tag, y_pred, target, offset)],
											on = .(locus_tag)]
...
aba_grid <- data.table::dcast(aba, spacer ~ factor(condition, levels = unique(condition)),
...
aba_y <- DGEList(counts = aba_grid_matrix, group = aba_group,
...
contrast_levels <- CJ(level2 = colnames(aba_permut), level1 = colnames(aba_permut))[
	level2 != level1,
...
melted_results_FDR <- results_FDR %>% 
	data.table::melt(
		id.vars = c("AB19606", "AB030", "unique_name", "spacer", "type", "y_pred", "target", "offset"),
...
melted_results <- melted_results_LFC %>% 
	melted_results_FDR[, on = c("AB19606", "AB030", "unique_name", "spacer", "type", "y_pred", "target", "offset", "condition")]
...
results_LFC <- dcast(melted_results[condition %in% interest$condition],
										 AB19606 + AB030 + unique_name + type + spacer + y_pred + target + offset ~ condition,
										 value.var = "LFC")
results_FDR <- dcast(melted_results[condition %in% interest$condition],
										 AB19606 + AB030 + unique_name + type + spacer + y_pred + target + offset ~ condition,
										 value.var = "FDR")

# Create median_results_LFC and median_results_FDR data.tables
median_results_LFC <- dcast(median_melted_results[condition %in% interest$condition],
														AB19606 + AB030 + unique_name + type ~ condition,
														value.var = "medLFC")
median_results_FDR <- dcast(median_melted_results[condition %in% interest$condition],
														AB19606 + AB030 + unique_name + type ~ condition,
														value.var = "FDR")
...
melted_results[, Pathway := case_when(AB030 %in% CellWall$AB030 ~ "Cell Wall/PG",
...
median_melted_results[, Pathway := case_when(AB030 %in% CellWall$AB030 ~ "Cell Wall/PG",
																						 AB030 %in% map03010$AB030 ~ "Ribosome",
																						 AB030 %in% LOS$AB030 ~ "LOS",
																						 AB030 %in% NADH$AB030 ~ "NADH",
																						 AB030 %in% GO0004812$AB030 ~ "tRNA Ligase",
...
median_melted_results[, gene_name_stylized := ifelse(unique_name %like% "GO593", unique_name, paste0("italic('", unique_name, "')"))]

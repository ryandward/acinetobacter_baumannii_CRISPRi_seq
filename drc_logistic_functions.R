BC.5.logistic <- function (fixed = c(NA, NA, NA, NA, NA), names = c("b", "c", "d", "e", "f"), ...) 
{
	numParm <- 5
	...
	return(braincousens_logistic(names = names, fixed = fixed, fctName = as.character(match.call()[[1]]), ...))
}

L.4.mod <- function (fixed = c(NA, NA, NA, NA), names = c("b", "c", "d", "e"), ...) 
{
	numParm <- 4
	if (!is.character(names) | !(length(names) == numParm)) {
		stop("Not correct 'names' argument")
	...
	return(mod_logistic(fixed = c(fixed, 1), names = c(names, "f"), fctName = as.character(match.call()[[1]]), ...))
}
		...
		ssfct = NULL, fctName, fctText) 
{
	...
	edfct <- function(parm, respl, reference, type, lower = 0.001, 
		...
		derParm <- c(tempVal * tempVal1 * (log(EDdose) - log(parmVec[4])), 
								 -parmVec[5] * EDdose/((tempVal2)^2), parmVec[5] * 
								 	EDdose/((tempVal2)^2), -tempVal * tempVal1 * 
								 	parmVec[1]/parmVec[4], -EDdose/tempVal2)

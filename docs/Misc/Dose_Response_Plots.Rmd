...
plot.graphic <- plot.fit_predictions %>% 
	...
	scale_fill_manual(values = c("#CAB2D6", "#B2DF8A", "#6A3D9A", "#33A02C")) +
	scale_colour_manual(values = c("#CAB2D6", "#B2DF8A", "#6A3D9A", "#33A02C")) +
...
```{r plot murC vulnerability for paper}
...
plot.fit_predictions <-
    ...
    mutate(
        Timing = case_when(
            ...
            Condition %like% "T2" ~ "T2"),
        Dose = case_when(
        	Condition %like% "0.06" | Condition %like% "0.11" ~ "Low",
        	Condition %like% "0.09" | Condition %like% "0.17" ~ "High",
        	Condition %like% "^None" ~ "No drug"),
        Dose = factor(Dose, levels = c("No drug", "Low", "High")),
        Drug = case_when(
            Condition %like% "^None_0" ~ "No drug",
            Condition %like% "Meropenem" ~ "Meropenem",
            Condition %like% "Imipenem" ~ "Imipenem"),
        Drug = factor(Drug, levels = c("No drug", "Meropenem", "Imipenem")))
...
plot.graphic <- plot.fit_predictions %>% 
	...
	geom_point(
		...
		size = 5,
			aes(
				...
				color = Dose)) + 
	geom_ribbon(
		data = plot.fit_predictions %>% filter(
			Gene %in% plot.genes &
				Condition %in% plot.conditions),
		alpha = 0.30,
		aes(
	...
	scale_colour_manual(values = c("grey", "#A6CEE3", "#1F78B4")) +
	xlab("Predicted Knockdown") +
	ylab("Log-2 Fitness Foldchange") +
	doc_theme +
	theme(legend.position = "bottom") +

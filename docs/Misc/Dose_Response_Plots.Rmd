...
plot.genes <- c("mraY", "murC", "murG")
plot.conditions <- c("Meropenem_0.17_T1 - None_0_T0")
plot.title <- bquote(bold(Cell ~ wall ~ genes ~ vulnerability ~ at ~ t[1] ~ 'in' ~ meropenem ~ '(CI = 90%)'))
...
plot.title <- bquote(bold(Gene ~ bolditalic(nuoM) ~ vulnerability ~ at ~ t[1] ~ '(CI = 90%)'))
plot.graphic <- fit_predictions %>% 
	filter(
		Gene %in% plot.genes &
			Condition %in% plot.conditions) %>%
...
```{r plot nuoF vulnerability}

plot.genes <- c("nuoF")
plot.conditions <- c("None_0_T1 - None_0_T0", "Rifampicin_0.34_T1 - None_0_T0", "Colistin_0.44_T1 - None_0_T0")
plot.title <- bquote(bold(Gene ~ bolditalic(nuoF) ~ vulnerability ~ at ~ t[1] ~ '(CI = 90%)'))
plot.graphic <- fit_predictions %>% 
	filter(
		Gene %in% plot.genes &
			Condition %in% plot.conditions) %>%
...
```{r plot nuoF-T2 vulnerability}

plot.genes <- c("nuoF")
plot.conditions <- c("None_0_T2 - None_0_T0", "Rifampicin_0.34_T2 - None_0_T0", "Colistin_0.44_T2 - None_0_T0")
plot.title <- bquote(bold(Gene ~ bolditalic(nuoF) ~ vulnerability ~ at ~ t[2] ~ '(CI = 90%)'))
plot.graphic <- fit_predictions %>% 
	filter(
		Gene %in% plot.genes &
			Condition %in% plot.conditions) %>%
...
```{r plot asd vulnerability}

plot.genes <- c("asd")
plot.conditions <- c("None_0_T2 - None_0_T0", "Imipenem_0.06_T2 - None_0_T0", "Imipenem_0.09_T2 - None_0_T0")
plot.title <- bquote(bold(Gene ~ bolditalic(asd) ~ vulnerability ~ at ~ t[1] ~ '(CI = 90%)'))
plot.graphic <- fit_predictions %>% 
	filter(
		Gene %in% plot.genes &
			Condition %in% plot.conditions) %>%

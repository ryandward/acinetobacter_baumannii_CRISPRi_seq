...
```{r Very vulnerable at T2}
	
plot.genes <- c("aroC", "murA", "ftsI")
plot.conditions <- c("None_0_T2 - None_0_T0")
plot.title <- bquote(bold("Very vulnerable" ~ at ~ t[2] ~ 'after Induction,' ~ 'CI = 0.9'))
...
```{r plot PG transferase vulnerability}
	
plot.genes <- c("murG", "murA", "mraY")
plot.conditions <- c("Imipenem_0.09_T1 - None_0_T0")
plot.title <- bquote(bold(PG~transferase~vulnerability ~ at ~ t[1] ~ 'in Imipenem 0.09,' ~ 'ci = 0.9'))
...
plot.genes <- c("murC", "murD", "murE", "murF", "murB")
plot.conditions <- c("Imipenem_0.09_T1 - None_0_T0")

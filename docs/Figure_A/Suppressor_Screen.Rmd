...
p_load(data.table, growthcurver, ggplot2, viridis)
...
suppressors[second_type == TRUE, `New Strain` := paste0(Strain, "R")]
...
suppressors[Strain != "empty" & Hour == 18] %>%
	ggplot( aes( x = `New Strain`, y = RFP_norm / OD600, fill = `New Strain`)) +
	geom_boxplot( outlier.colour = NA ) +
	scale_fill_viridis( discrete = TRUE, alpha = 0.5) +
	geom_jitter() +
	ggtitle(this_title) +

...
aba.genome[AB030 %in% LPS$AB030, Pathway := "LOS"]
...
```{r Perform edgeR for Rifampicin and Colistin Samples}
aba.contrasts <- c(
	"Rifampicin_0.34_T1 - None_0_T1",
	"Rifampicin_0.34_T2 - None_0_T2",
	"Colistin_0.44_T1 - None_0_T1",
...
median_LFC <- dcast(
	melted_results[(Pathway == "LOS" | Pathway == "NADH")  & type == "perfect"], 
	unique_name + Pathway ~ condition, 
	value.var = "LFC.adj", 
	fun.aggregate = median)
...
to_plot_title <- "Log-2 Fold Fitness Change in Knockdown Strain"
...
# format conditions (columns)
conditions.formatted <- c(
	bquote(Rifampicin ~ (hour[9])),
	bquote(Rifampicin ~ (hour[18])),
	bquote(Colistin ~ (hour[9])),
	bquote(Colistin ~ (hour[18])))

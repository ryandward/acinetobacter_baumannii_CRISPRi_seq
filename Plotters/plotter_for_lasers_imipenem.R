...
melted_results %>% 
	...
	filter(condition %in% c("Imipenem_0.09_T1 - None_0_T1", "None_0_T1 - None_0_T0")) %>% 
	...
	ggplot(aes(x = Inducer, y = Imipenem)) + 
...
imipenem_subset <- melted_results %>% 
	filter(type == "mismatch") %>%
...
imipenem_subset.cors <- imipenem_subset %>%
	summarise(
		p.cor = cor.test(Imipenem, Inducer)$p.value,
		# cor = cor.test(Imipenem, Inducer)$statistic, 
		r = cor.test(Imipenem, Inducer)$estimate,
		r_sq = cor.test(Imipenem, Inducer)$estimate^2,
		# method = cor.test(Imipenem, Inducer)$method,
		df.guides = cor.test(Imipenem, Inducer)$parameter)

imipenem_subset.regressions <- imipenem_subset %>% 
	nest(data = -Pathway) %>%
	mutate(
		fit = map(data, ~ lm(Imipenem ~ Inducer + 0, data = .x)),
		tidied = map(fit, tidy)
	) %>% 
	unnest(tidied)

imipenem.stats <- imipenem_subset.cors %>% 
	inner_join(imipenem_subset.regressions) %>% 
	select(Pathway, r_sq, p.cor, estimate, std.error, p.value, df.guides) %>% 
	rename (slope = estimate, std.err.slope = std.error, p.slope = p.value) %>% print
...
imipenem.stats <- imipenem.stats %>%
	rbind(imipenem_subset.cors %>% 
	inner_join(imipenem_subset.regressions) %>% 
	select(Pathway, r_sq, p.cor, estimate, std.error, p.value, df.guides) %>% 
	rename (slope = estimate, std.err.slope = std.error, p.slope = p.value))

imipenem.stats %>% print
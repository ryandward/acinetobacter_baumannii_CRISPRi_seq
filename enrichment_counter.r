...
contrasts <- makeContrasts(
  ...
  T2_Colistin = T2_Colistin - T2_None,
  T1_Imipenem = T1_Imipenem - T1_None,
  T2_Imipenem = T2_Imipenem - T2_None,
  T1_Meropenem = T1_Meropenem - T1_None,
  T2_Meropenem = T2_Meropenem - T2_None,
...
create_plot <- function(full_data, targets, enrichments, sets) {
  ...
  plot_data <- full_data %>%
    ...
    inner_join(enrichments, relationship = "many-to-many") %>%
    inner_join(sets %>% arrange(FDR) %>% head(12)) %>%
    filter(FDR <= 0.05) %>%
    ...
    mutate(facet_title = paste(facet_title, paste0("**FDR** = ", signif(FDR, 2), ", *n* = ", guide_count), contrast, sep = "<br>")) %>%
...
plot_results <- function(results, targets, enrichments, sets) {
  ...
  ggplot(plot_data, aes(y = logFC, x = factor(contrast))) +
    # geom_tile(data = data.frame(timing= "T2"), aes(x = timing, y = 0), width = 1, height = Inf, fill = "grey50", alpha = 0.2, inherit.aes = FALSE) +
    geom_sina(aes(weight = as.numeric(-log10(adj.P.Val)), color = factor(contrast), size = -log10(adj.P.Val)), alpha = 0.5, shape = 16, scale = "width") +
    geom_violin(aes(weight = as.numeric(-log10(adj.P.Val))), alpha = 0.35, draw_quantiles = c(0.25, 0.5, 0.75), position = "dodge", scale = "width") +
    facet_wrap(~facet_title, nrow = 3, scales = "free_y") +
    scale_size(range = c(0.25, 2.5)) +
    ggtitle(set_name) +

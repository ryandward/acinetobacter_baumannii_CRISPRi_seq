...
aba_fit <- glmFit(aba_y, aba_permut, robust = TRUE)
...
for (i in 1:ncol(aba_contrast)) {
	...
	results <- glmLRT(aba_fit, contrast = aba_contrast[,i])
...
fwrite(results_LFC, "Results/results_nonQL_LFC.tsv.gz", sep = "\t")

fwrite(results_FDR, "Results/results_nonQL_FDR.tsv.gz", sep = "\t")

fwrite(median_results_LFC, "Results/median_results_nonQL_LFC.tsv.gz", sep = "\t")

fwrite(median_results_FDR, "Results/median_results_nonQL_FDR.tsv.gz", sep = "\t")
...
fwrite(melted_results, "Results/melted_results_nonQL_.tsv.gz", sep = "\t")

fwrite(median_melted_results, "Results/median_melted_results_nonQL.tsv.gz", sep = "\t")

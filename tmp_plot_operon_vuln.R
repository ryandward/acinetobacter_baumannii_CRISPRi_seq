...
plot.operons <- c("lpxC", "nuoABDEFGHIJKLMN")
...
plot.title <- bquote(bold("Operon dose effect on drug activity" ~ at ~ hour[36] ~ '(Confidence = 0.90)'))

plot.graphic <- plot.fit_predictions %>% 
	filter(
		Operon %in% plot.operons &
	...
	geom_line(
		...
		aes(
			x = y_pred, 
			y = .fitted, 
			color = interaction(Timing, Operon))) +
	geom_point(
	...
	geom_ribbon(
		...
		aes(
			...
			ymin = .lower,
			ymax = .upper,
			fill = interaction(Timing, Operon))) +
	scale_fill_manual(
		values = c(
			"No drug" = "white",
			"Colistin" = "#A6CEE3",
			"Rifampicin" = "#FB9A99",
			"T1.nuoABDEFGHIJKLMN" = "#CAB2D6",
			"T2.nuoABDEFGHIJKLMN" = "#6A3D9A",
			"T1.lpxC" = "#B2DF8A",
			"T2.lpxC" = "#33A02C")) +
	scale_color_manual(
		values = c(
	...
	facet_grid	(
		facets = c("Operon", "Drug"), scales = "free") + 
